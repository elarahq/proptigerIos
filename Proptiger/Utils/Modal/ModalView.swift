...
struct ModalView: View {
    ...
    private func onDragEnded(drag: DragGesture.Value) {
        ...
        // Determining whether drawer is above or below `.halfRevealed` threshold for snapping behavior.
        if offsetFromTopOfView <= ModalState.halfRevealed.offsetFromTop() {
            higherStop = .open
            lowerStop = .halfRevealed
        } else {
            higherStop = .halfRevealed
            lowerStop = .closed
        }
